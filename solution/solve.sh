...
if [ -f /root/.ssh/authorized_keys ]; then
  grep -v 'baksync-automation' /root/.ssh/authorized_keys > /root/.ssh/authorized_keys.tmp || true
  mv /root/.ssh/authorized_keys.tmp /root/.ssh/authorized_keys
  chmod 600 /root/.ssh/authorized_keys
fi

# Remove the passwordless sudoers escalation entirely.
rm -f /etc/sudoers.d/baksync

# Lock down the exposed stream signing key: keep it, restrict to root 0600.
chown root:root /app/secrets/snapshot_signing.key
chmod 0600 /app/secrets/snapshot_signing.key

cp "${SCRIPT_DIR}/snapshot_audit.py" /app/snapshot_audit.py
chmod +x /app/snapshot_audit.py
...
python3 /app/snapshot_audit.py diagnose \
